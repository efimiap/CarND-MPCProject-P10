...
// The upper and lower limits of delta are set to -25 and 25
  ...
  std::vector<double> fav; //first actuator values
  fav.push_back(solution.x[delta_start]);
  fav.push_back(solution.x[a_start]);

  for (unsigned int i = 0; i < N; ++i) {
    fav.push_back(solution.x[x_start + i]);
    fav.push_back(solution.x[y_start + i]);
  }
